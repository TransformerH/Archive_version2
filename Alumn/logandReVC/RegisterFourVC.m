...
@property (nonatomic,strong) NSArray *stateTryArray;
@property (nonatomic,strong) NSArray *cityArray;
@property (nonatomic,strong) NSArray *cityTryArray;

@property  NSInteger countryIndex;
...
- (IBAction)choosePlace:(id)sender {
    ...
    NSInteger countryRow = [self.placePicker selectedRowInComponent:countryPicker];
    NSInteger stateRow = [self.placePicker selectedRowInComponent:statePicker];
    NSInteger cityRow = [self.placePicker selectedRowInComponent:cityPicker];
    
    NSString *selectCountry = self.countryArray[countryRow];
    NSString *selectState = self.stateTryArray[stateRow];
    NSString *selectCity = self.cityTryArray[cityRow];
    
    self.countryText.text = selectCountry;
    self.stateText.text = selectState;
    self.cityText.text = selectCity;
    
    self.stateTryArray = @[@""];
    self.cityTryArray = @[@""];
    
    [self.placePicker reloadComponent:cityPicker];
    [self.placePicker reloadComponent:statePicker];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component{
    ...
    switch(component){
        case countryPicker:
            ...
            break;
        case statePicker:
            count = [self.stateTryArray count];
            break;
        case cityPicker:
            count = [self.cityTryArray count];
            break;
    }
...
- (NSString*)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component{
    ...
    switch(component){
        case countryPicker:
            data = self.countryArray[row];
            break;
        case statePicker:
            data = self.stateTryArray[row];
            break;
        case cityPicker:
            data = self.cityTryArray[row];
            break;
    }
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    if(component == countryPicker){
        self.stateTryArray = self.stateArray[row];
        [self.placePicker reloadComponent:statePicker];
        [self.placePicker selectRow:0 inComponent:statePicker animated:YES];
        self.countryIndex = row;
    }
    if(component == statePicker){
        
        NSInteger cityNum = 0;
        for(int i = 0;i < self.countryIndex;i++){
            NSInteger state = [[self.stateArray objectAtIndex:i] count];
            if(state == 0){
                cityNum += 0;
            }else{
                cityNum += state;
        ...
        self.cityTryArray =  self.cityArray[cityNum+row];
        [self.placePicker reloadComponent:cityPicker];
        [self.placePicker selectRow:0 inComponent:cityPicker animated:YES];
    }
}
