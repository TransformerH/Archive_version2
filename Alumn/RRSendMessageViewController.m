...
- (void) postMessage {
    ...
    if([[self.selectedPhotos valueForKeyPath:@"@min.floatValue"] floatValue]>0)
    {
...
//    if(countPics >= imgNum){
    [User upLoadImages: self.selectedPhotos UploadSuccess:^(NSString *str, BOOL success) {
        if(str.length>0&&self.textView.text.length>0)
        {
        ...
        [Dynamic publishWithParameters:upLoadDic SuccessBlock:^(NSDictionary *dict, BOOL success) {
            ...
            UIAlertView *alert2 = [[UIAlertView alloc] initWithTitle:@"提示" message:@"发送文本长度应该大于0，请重试" delegate:self cancelButtonTitle:@"确认" otherButtonTitles:nil];
            [alert2 show];
            if ([self.delegate respondsToSelector:@selector(messageCancel)]) {
                [self.delegate messageCancel];
            }
...
//    }
        ...
        UIAlertView *alert2 = [[UIAlertView alloc] initWithTitle:@"提示" message:@"发送图片张数需要大于0，请重试" delegate:self cancelButtonTitle:@"确认" otherButtonTitles:nil];
        [alert2 show];
        if ([self.delegate respondsToSelector:@selector(messageCancel)]) {
            [self.delegate messageCancel];
        }
